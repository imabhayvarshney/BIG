...
if [[ ${TELEMETRY_ENABLED} = "false" ]] || [ -z "${TELEMETRY_ENABLED}" ] ;then
    docker-compose -f bigid-all-compose.yml -f bigid-ner-compose.yml ${*:-up -d}
else
    if [ -n "${MONITORING_CUSTOMER_NAME}" ];then
        docker-compose -f bigid-all-compose.yml -f bigid-newrelic-compose.yml -f bigid-ner-compose.yml ${*:-up -d}
    else
