#!/usr/bin/env bash
echo "Running BigID (config: all)"
source setenv.sh
if [[ ${TELEMETRY_ENABLED} = "false" ]] || [ -z "${TELEMETRY_ENABLED}" ] ;then
    docker-compose -f bigid-ext-mongo-compose.yml -f bigid-confluent-compose.yml ${*:-up -d}
else
    if [ -n "${MONITORING_CUSTOMER_NAME}" ];then
        docker-compose -f bigid-ext-mongo-compose.yml -f bigid-confluent-compose.yml -f bigid-newrelic-compose.yml ${*:-up -d}
    else
