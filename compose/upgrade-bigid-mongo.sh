...
# ./upgrade_mongo.sh 7.0 bigid password
...
docker run -d -p 27017:27017 -v bigid-mongo-data:/data/db --name bigid-mongo mongo:$TARGET_VERSION
sleep 10
...
while [ $COUNTER -lt 10 ]; do
    ...
    MONGOCONNSTAT=$?
    MONGOTEST=$(docker exec bigid-mongo mongosh --eval "print('test')" 2>&1)
    MONGOTESTSTAT=$?
