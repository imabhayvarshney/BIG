...
if [[ ${TELEMETRY_ENABLED} = "false" ]] || [ -z "${TELEMETRY_ENABLED}" ] ;then
    docker-compose -f bigid-ext-mongo-compose.yml -f bigid-reporting-etl-compose.yml -f bigid-ext-reporting-etl-compose.yml ${*:-up -d}
else
    if [ -n "${MONITORING_CUSTOMER_NAME}" ];then
        docker-compose -f bigid-ext-mongo-compose.yml -f bigid-reporting-etl-compose.yml -f bigid-ext-reporting-etl-compose.yml -f bigid-newrelic-compose.yml ${*:-up -d}
    else
